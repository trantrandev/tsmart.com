...
class AdminUserController extends Controller
{
    ...
    function store(Request $request)
    {
        ...
        if ($request->hasFile('avatar')) {
            ...
            if (!file_exists(public_path('admin/images/users')) && !mkdir($concurrentDirectory = public_path('admin/images/users'), 0777, true) && !is_dir($concurrentDirectory)) {
...
// Thông báo
    ...
    function action(Request $request)
    {
        ...
        // ! Nếu có chọn action
        if ($list_check && $request->input('act')) {
