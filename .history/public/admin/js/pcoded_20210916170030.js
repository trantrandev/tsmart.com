...
$(document).ready(function() {
    ...
    $("#m-search").on("keyup", function() {
        ...
        $(".pcoded-inner-navbar > li").each(function() {
            ...
            if (t) { var s = t.toLowerCase(); }
            if (s) {
                var n = s.indexOf(g);
                if (n !== -1) { $(this).show(); } else { $(this).hide(); }
                if (ln > 0) { $('.pcoded-menu-caption').hide(); } else { $('.pcoded-menu-caption').show(); }
            }
        });
    });
    ...
    $('.h-send-chat').on('keyup', function(e) { fc(e); });
    $('.btn-send').on('click', function(e) { cfc(e); });
    if (vw <= 991) {
        $(".main-search").addClass('open');
        $(".main-search .form-control").css({ 'width': '90px' });
    }
    if (vw >= 1024) { var px = new PerfectScrollbar('.main-friend-cont', { wheelSpeed: .5, swipeEasing: 0, suppressScrollX: !0, wheelPropagation: 1, minScrollbarLength: 40, }); var px = new PerfectScrollbar('.main-chat-cont', { wheelSpeed: .5, swipeEasing: 0, suppressScrollX: !0, wheelPropagation: 1, minScrollbarLength: 40, }); }
    if (!$('.pcoded-navbar').hasClass('theme-horizontal')) { var vw = $(window)[0].innerWidth; if (vw < 992 || $('.pcoded-navbar').hasClass('menupos-static')) { var px = new PerfectScrollbar('.navbar-content', { wheelSpeed: .5, swipeEasing: 0, suppressScrollX: !0, wheelPropagation: 1, minScrollbarLength: 40, }); } else { var px = new PerfectScrollbar('.navbar-content', { wheelSpeed: .5, swipeEasing: 0, suppressScrollX: !0, wheelPropagation: 1, minScrollbarLength: 40, }); } }

    function fc(e) { if (e.which == 13) { cfc(e) } };
    ...
    function frc(wrmsg) {
        setTimeout(function() {
            $('.header-chat .main-friend-chat').append('' +
                ...
                '<a class="media-left photo-table" href="#!"><img class="media-object img-radius img-radius m-t-5" src="public/min/images/user/avatar-2.jpg" alt="Generic placeholder image"></a>' +
    ...
    $(".card-option .close-card").on('click', function() {
        ...
        $this.parents('.card').animate({ 'margin-bottom': '0', });
        setTimeout(function() {
        ...
        setTimeout(function() { $this.parents('.card').remove(); }, 1500);
    });
    ...
    $(".card-option .minimize-card").on('click', function() {
        ...
        if (!port.hasClass('full-card')) { $(port).css("height", "auto"); }
    ...
    $(".card-option .full-card").on('click', function() {
        ...
        if (port.hasClass('full-card')) {
            ...
            port.animate({ 'marginLeft': l - (l * 2), 'marginTop': t - (t * 2), 'width': docW, 'height': docH, });
        } else {
            $('body').css('overflow', '');
            port.removeAttr('style');
            setTimeout(function() { $('html,body').animate({ scrollTop: $(port).offset().top }, 500); }, 400);
        }
    });
...
$(window).scroll(function() {
    ...
    if (vw >= 768) {
        ...
        if (cOst == 400) { $('.theme-horizontal').addClass('top-nav-collapse'); } else if (cOst > ost && 400 < ost) { $('.theme-horizontal').addClass('top-nav-collapse').removeClass('default'); } else { $('.theme-horizontal').addClass('default').removeClass('top-nav-collapse'); }
        ost = cOst;
    }
});
...
function togglemenu() {
    ...
    if ($(".pcoded-navbar").hasClass('theme-horizontal') == false) {
        if (vw <= 1200 && vw >= 992) { $(".pcoded-navbar").addClass("navbar-collapsed"); }
        if (vw < 992) { $(".pcoded-navbar").removeClass("navbar-collapsed"); }
    }
}
...
$.fn.pcodedmenu = function(settings) {
    ...
    var defaults = { themelayout: 'vertical', MenuTrigger: 'click', SubMenuTrigger: 'click', };
        ...
        HandleSubMenuTrigger: function() {
            ...
            if ($('.pcoded-navbar').hasClass('theme-horizontal') == true) {
                if (newSize >= 768) {
                    ...
                    $dropdown.off('click').off('mouseenter mouseleave').hover(function() { $(this).addClass('pcoded-trigger'); }, function() { $(this).removeClass('pcoded-trigger'); });
                } else {
...
$(window).ready(function() {
    var vw = $(window)[0].innerWidth;
    $(".pcoded-navbar").on('click tap', function(e) { e.stopPropagation(); });
